...
static unsigned long kasumi_resolve_static_symbol(const char *name)
{
	extern struct mm_struct init_mm __attribute__((weak));
	extern void __set_fixmap(enum fixed_addresses idx, phys_addr_t phys, pgprot_t prot) __attribute__((weak));
	extern struct rw_semaphore uts_sem __attribute__((weak));
	extern struct uts_namespace init_uts_ns __attribute__((weak));
	extern struct task_struct init_task __attribute__((weak));
	extern struct filename *getname_flags(const char __user *filename, int flags, int *empty) __attribute__((weak));
	extern struct kmem_cache *names_cachep __attribute__((weak));
	extern void emergency_sync(void) __attribute__((weak));
	extern char *d_absolute_path(const struct path *path, char *buf, int buflen) __attribute__((weak));
	extern int vfs_getattr(const struct path *, struct kstat *, u32, unsigned int) __attribute__((weak));
	extern char *d_path(const struct path *, char *, int) __attribute__((weak));
	extern int iterate_dir(struct file *, struct dir_context *) __attribute__((weak));
	extern ssize_t vfs_getxattr(struct dentry *, const char *, void *, size_t) __attribute__((weak));
	extern int unshare_nsproxy_namespaces(unsigned long, struct nsproxy **, struct cred *, struct fs_struct *) __attribute__((weak));
	extern void switch_task_namespaces(struct task_struct *p, struct nsproxy *new) __attribute__((weak));
	extern struct nsproxy *create_new_namespaces(unsigned long flags, struct task_struct *tsk, struct user_namespace *user_ns, struct fs_struct *new_fs) __attribute__((weak));

	extern void __flush_dcache_area(void *addr, size_t len) __attribute__((weak));

	extern char selinux_blob_sizes __attribute__((weak));

	extern bool ksu_uid_should_umount(uid_t uid) __attribute__((weak));
	extern bool __ksu_is_allow_uid_for_current(uid_t uid) __attribute__((weak));
	extern bool __ksu_is_allow_uid(uid_t uid) __attribute__((weak));
	extern void *ksu_syscall_table __attribute__((weak));
	extern int ksu_dispatcher_nr __attribute__((weak));
	extern bool ksu_get_allow_list(int *array, u16 length, u16 *out_length, u16 *out_total, bool allow) __attribute__((weak));

	if (strcmp(name, "init_mm") == 0)
		return &init_mm ? (unsigned long)&init_mm : 0;
	if (strcmp(name, "__set_fixmap") == 0)
		return &__set_fixmap ? (unsigned long)&__set_fixmap : 0;
	if (strcmp(name, "uts_sem") == 0)
		return &uts_sem ? (unsigned long)&uts_sem : 0;
	if (strcmp(name, "init_uts_ns") == 0)
		return &init_uts_ns ? (unsigned long)&init_uts_ns : 0;
	if (strcmp(name, "init_task") == 0)
		return &init_task ? (unsigned long)&init_task : 0;
	if (strcmp(name, "getname_flags") == 0)
		return &getname_flags ? (unsigned long)&getname_flags : 0;
	if (strcmp(name, "names_cachep") == 0)
		return &names_cachep ? (unsigned long)&names_cachep : 0;
	if (strcmp(name, "emergency_sync") == 0)
		return &emergency_sync ? (unsigned long)&emergency_sync : 0;
	if (strcmp(name, "d_absolute_path") == 0)
		return &d_absolute_path ? (unsigned long)&d_absolute_path : 0;
	if (strcmp(name, "vfs_getattr") == 0)
		return &vfs_getattr ? (unsigned long)&vfs_getattr : 0;
	if (strcmp(name, "d_path") == 0)
		return &d_path ? (unsigned long)&d_path : 0;
	if (strcmp(name, "iterate_dir") == 0)
		return &iterate_dir ? (unsigned long)&iterate_dir : 0;
	if (strcmp(name, "vfs_getxattr") == 0)
		return &vfs_getxattr ? (unsigned long)&vfs_getxattr : 0;
	if (strcmp(name, "unshare_nsproxy_namespaces") == 0)
		return &unshare_nsproxy_namespaces ? (unsigned long)&unshare_nsproxy_namespaces : 0;
	if (strcmp(name, "switch_task_namespaces") == 0)
		return &switch_task_namespaces ? (unsigned long)&switch_task_namespaces : 0;
	if (strcmp(name, "create_new_namespaces") == 0)
		return &create_new_namespaces ? (unsigned long)&create_new_namespaces : 0;

	if (strcmp(name, "selinux_blob_sizes") == 0)
		return &selinux_blob_sizes ? (unsigned long)&selinux_blob_sizes : 0;

	if (strcmp(name, "__flush_dcache_area") == 0)
		return &__flush_dcache_area ? (unsigned long)&__flush_dcache_area : 0;

	if (strcmp(name, "ksu_uid_should_umount") == 0)
		return &ksu_uid_should_umount ? (unsigned long)&ksu_uid_should_umount : 0;
	if (strcmp(name, "__ksu_is_allow_uid_for_current") == 0)
		return &__ksu_is_allow_uid_for_current ? (unsigned long)&__ksu_is_allow_uid_for_current : 0;
	if (strcmp(name, "__ksu_is_allow_uid") == 0)
		return &__ksu_is_allow_uid ? (unsigned long)&__ksu_is_allow_uid : 0;
	if (strcmp(name, "ksu_syscall_table") == 0)
		return &ksu_syscall_table ? (unsigned long)&ksu_syscall_table : 0;
	if (strcmp(name, "ksu_dispatcher_nr") == 0)
		return &ksu_dispatcher_nr ? (unsigned long)&ksu_dispatcher_nr : 0;
	if (strcmp(name, "ksu_get_allow_list") == 0)
		return &ksu_get_allow_list ? (unsigned long)&ksu_get_allow_list : 0;
...
{
	unsigned long static_addr = kasumi_resolve_static_symbol(name);
	if (static_addr)
		return static_addr;
...
{
	pr_alert("Kasumi: resolving kallsyms_lookup_name directly...\n");
	kasumi_kallsyms_lookup_name = kallsyms_lookup_name;
