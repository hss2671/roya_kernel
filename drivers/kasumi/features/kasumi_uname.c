...
void kasumi_uname_get_effective(char *release, char *version)
{
	struct kasumi_uname_cfg_rcu *cfg;

	if (release)
		strscpy(release, kasumi_init_uts_ns_ptr ? kasumi_init_uts_ns_ptr->name.release : init_uts_ns.name.release, __NEW_UTS_LEN + 1);
	if (version)
		strscpy(version, kasumi_init_uts_ns_ptr ? kasumi_init_uts_ns_ptr->name.version : init_uts_ns.name.version, __NEW_UTS_LEN + 1);

	rcu_read_lock();
	cfg = rcu_dereference(kasumi_uname_scoped_cfg);
	if (cfg) {
		if (release && field_set(cfg->data.release))
			strscpy(release, cfg->data.release, __NEW_UTS_LEN + 1);
		if (version && field_set(cfg->data.version))
			strscpy(version, cfg->data.version, __NEW_UTS_LEN + 1);
	}
	rcu_read_unlock();
}
