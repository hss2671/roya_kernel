...
void kasumi_hook_vfs_getattr(const struct path *path, struct kstat *stat)
{
	char buf[256];
	char *dp;
	...
	if (atomic_long_read(&kasumi_ioctl_tgid) == (long)task_tgid_vnr(current))
		return;
	if (kasumi_this_cpu() && kasumi_this_cpu()->in_populate_inject)
		return;
	if (atomic_read(&kasumi_rule_count) == 0 &&
	    atomic_read(&kasumi_spoof_kstat_count) == 0)
		return;
	...
	mapping = inode ? inode->i_mapping : NULL;

	if (mapping && test_bit(AS_FLAGS_KASUMI_SPOOF_KSTAT, &mapping->flags)) {
		is_target = true;
	} else {
		dp = ERR_PTR(-ENOENT);
		if (kasumi_d_absolute_path)
			dp = kasumi_d_absolute_path(path, buf, sizeof(buf));
		if (IS_ERR(dp) && kasumi_dentry_path_raw)
			dp = kasumi_dentry_path_raw(path->dentry, buf, sizeof(buf));
		...
		if (!IS_ERR_OR_NULL(dp) && dp[0] == '/') {
			...
			if (!is_target && inode) {
				if (kasumi_spoof_kstat_lookup_by_ino(
					...
					inode->i_sb ? (unsigned long)inode->i_sb->s_dev : 0))
					is_target = true;
			}
			rcu_read_unlock();
		}
