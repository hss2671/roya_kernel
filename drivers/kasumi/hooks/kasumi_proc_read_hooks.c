					 ...
					 size_t count, loff_t *ppos)
{
	...
	if (proxy->kind == KASUMI_PROC_PROXY_OSRELEASE ||
	    proxy->kind == KASUMI_PROC_PROXY_VERSION) {
		char eff_release[65] = {0};
		char eff_version[65] = {0};
		const char *src_str;
		char tmp_buf[128];
		size_t str_len;
		loff_t current_pos = ppos ? *ppos : file->f_pos;

		extern void kasumi_uname_get_effective(char *release, char *version);
		kasumi_uname_get_effective(eff_release, eff_version);
		...
		str_len = scnprintf(tmp_buf, sizeof(tmp_buf), "%s\n", src_str);
		...
		if (count > str_len - current_pos)
			count = str_len - current_pos;

		if (copy_to_user(buf, tmp_buf + current_pos, count)) {
			ret = -EFAULT;
			goto out;
		}
					      ...
					      struct iov_iter *to)
{
	...
	if (proxy->kind == KASUMI_PROC_PROXY_OSRELEASE ||
		...
		char tmp_buf[128];
		size_t str_len;
		loff_t current_pos = iocb->ki_pos;
		...
		bytes_to_copy = str_len - current_pos;
		if (iov_iter_count(to) < bytes_to_copy)
			bytes_to_copy = iov_iter_count(to);

		if (copy_to_iter(tmp_buf + current_pos, bytes_to_copy, to) != bytes_to_copy) {
			ret = -EFAULT;
			goto out;
		}

		iocb->ki_pos += bytes_to_copy;
		ret = bytes_to_copy;
		goto out;
	}
...
KASUMI_NOCFI int kasumi_mount_proxy_install_fd_explicit(int fd, enum kasumi_proc_proxy_kind kind)
{
	struct file *file;
	struct kasumi_mount_file_proxy *proxy;
	const struct file_operations *new_fops;
	...
	kasumi_log("proc_proxy: installed fd=%d kind=%d (explicit) pid=%d comm=%s\n",
		 fd, kind, task_pid_nr(current), current->comm);
...
void kasumi_proc_read_hooks_init(void)
{
	...
	bool use_proxy_filter = kasumi_has_syscall_hook(__NR_openat);
