...
kasumi_syscall_hook_fn orig_kernel_openat, orig_kernel_statfs, orig_kernel_fstatfs;
#ifdef __NR_openat2
kasumi_syscall_hook_fn orig_kernel_openat2;
#endif
#ifdef __NR_statx
...
static long do_openat(const struct pt_regs *regs, kasumi_syscall_hook_fn orig)
{
	...
	enum kasumi_proc_proxy_kind uts_proxy_kind = KASUMI_PROC_PROXY_NONE;
	...
	if (path[0] == '/') {
		if (strcmp(path, "/proc/sys/kernel/osrelease") == 0) {
			uts_proxy_kind = KASUMI_PROC_PROXY_OSRELEASE;
		} else if (strcmp(path, "/proc/sys/kernel/version") == 0) {
			uts_proxy_kind = KASUMI_PROC_PROXY_VERSION;
		}
	}

	if (uts_proxy_kind != KASUMI_PROC_PROXY_NONE) {
		char __user *n = kasumi_userspace_stack_buffer("/dev/null", sizeof("/dev/null"));
		if (n)
	...
	if (path[0] == '/' && atomic_read(&kasumi_rule_count) > 0 &&
	    uts_proxy_kind == KASUMI_PROC_PROXY_NONE) {
	...
	ret = orig(regs);
	if (uts_proxy_kind != KASUMI_PROC_PROXY_NONE && ret >= 0) {
		kasumi_mount_proxy_install_fd_explicit((int)ret, uts_proxy_kind);
	}
	if (!raw_proc_proxy && uts_proxy_kind == KASUMI_PROC_PROXY_NONE && ret >= 0 && target_path)
...
int kasumi_syscall_redirect_init(void)
{
...

	{
		...
		kasumi_syscall_table = (void *)sys_call_table;
	}
