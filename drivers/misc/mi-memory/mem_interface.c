...
{
	int offset = 9;
/*M17-T code for HQ-254025 by gaoshilin at 22.11.15 start*/
#ifdef CONFIG_MI_MEMORY_SYSFS
	if(ufs_data_ptr == NULL){
		set_ufs_hba_data(NULL);
	}
#endif
/*M17-T code for HQ-254025 by gaoshilin at 22.11.15 end*/
	ufs_data_ptr->ufs_err_state.err_occurred++;
#ifdef CONFIG_BUILD_QGKI
#ifdef CONFIG_DEBUG_FS
    // For Compatible with Memory debugfs interface
/*M17-T code for HQ-254025 by gaoshilin at 22.11.15 start*/
    if ((ufs_data_ptr->hba != NULL)
	   &&(!ufs_data_ptr->hba->debugfs_files.err_occurred)
